...
const createAndSavePerson = (done) => {
  const ryan = new Person({
    name: 'Ryan',
    age: 35,
    favoriteFoods: ['cheeseburger', 'steak', 'hot dogs']
  })
...
const arrayOfPeople = [
  ...
  name: 'Jim',
  age: 39,
  favoriteFoods: ['burnt steak', 'burn chicken', 'IPA beer']
  },
  {
  name: 'Andy',
  age: 32,
  favoriteFoods: ['Steamed dodo bird', 'dinosaur eggs', 'warm milk']
  },
  {
  name: 'Jessie',
  age: 38,
  favoriteFoods: ['steak', 'cheese puffs', 'lindor truffles']
  },
  {
  name: 'Mike',
  age: 39,
  favoriteFoods: ['steak', 'shrimp', 'lobster']
  }
]
...
const findAndUpdate = function(personName, done) {
  ...
  Person.findOneAndUpdate(personName, ageToSet, (err, data) => {
    if(err){
      done(err)
    }
    data.age = ageToSet
